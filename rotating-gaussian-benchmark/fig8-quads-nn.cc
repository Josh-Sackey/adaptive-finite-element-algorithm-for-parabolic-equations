...
namespace RotatingGaussian
{
  ...
  public:
    ...
    {
      const double cx = coordinate_shift +
                        radius * std::cos(omega * this->get_time());
      const double cy = coordinate_shift +
                        radius * std::sin(omega * this->get_time());
  ...
  public:
                 ...
                 const unsigned int = 0) const override
    {
      ...
      const double cx = coordinate_shift + radius * std::cos(omega * t);
      const double cy = coordinate_shift + radius * std::sin(omega * t);
  ...
  private:
    ...
    void refine_without_transfer(const Vector<float> &indicators,
                                 const double refinement_fraction);
    ...
    const unsigned int figure8_target_dofs = 12818;
    const unsigned int max_target_refinements = 8;
  ...
  void Problem::adapt_mesh()
  {
    ...
    // Permit the concentrated square mesh to approach the resolution of the
    // 12,818-vertex final mesh reported for Figure 8.
  ...
  TrainingResult Problem::train_surrogate(const double time)
  {
    ...
    // Figure 5 reports more than 2,000 epochs for the initial fit and fewer
    // than 100 at later time levels. Give LBFGS a comparable initial budget;
    // its stopping criteria may still terminate earlier.
    ...
    for (unsigned int cycle = 0; cycle < 5; ++cycle)
      {
        ...
        if (cycle < 4)
          ...
          {
            // The paper's SIZE/GENERATE stage makes a large fifth-step jump
            // from 1,388 to 12,818 vertices. Hierarchical quadrilaterals do
            // not provide that mesh generator, so reproduce its resolution
            // target with repeated recovery-guided bulk refinements while
            // retaining only the fifth mesh as the observable output stage.
            unsigned int target_refinement = 0;
            while (dof_handler.n_dofs() < figure8_target_dofs &&
                   target_refinement < max_target_refinements)
              {
                const double remaining_ratio =
                  static_cast<double>(figure8_target_dofs) /
                    dof_handler.n_dofs() -
                  1.0;
                const double target_fraction =
                  std::min(top_fraction,
                           std::max(0.05, remaining_ratio / 2.2));
                refine_without_transfer(recovery_indicators(), target_fraction);
                assemble_initial_projection();
                solve();
                ++target_refinement;
              }
